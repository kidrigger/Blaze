...
LightSystem::LightSystem(const Context& context) noexcept
{
	...
	renderPassOmni = Managed(createRenderPassMultiView(context.get_device(), 0b00111111, format, VK_FORMAT_D32_SFLOAT,
													   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL),
							 [dev = context.get_device()](VkRenderPass& rp) { vkDestroyRenderPass(dev, rp, nullptr); });
	renderPassDirectional =
	...
	{
		{
			std::vector<VkDescriptorPoolSize> poolSizes = {
				...
				{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_POINT_LIGHTS + MAX_DIR_LIGHTS}};
			dsPool = util::Managed(util::createDescriptorPool(context.get_device(), poolSizes, 17),
			...
			dsLayout = util::Managed(util::createDescriptorSetLayout(context.get_device(), bindings),
									 [dev = context.get_device()](VkDescriptorSetLayout& lay) {
										 vkDestroyDescriptorSetLayout(dev, lay, nullptr);
									 });

			bindings = {
				{0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_POINT_LIGHTS, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr},
				{1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_DIR_LIGHTS, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr}};
			shadowLayout = util::Managed(util::createDescriptorSetLayout(context.get_device(), bindings),
										 [dev = context.get_device()](VkDescriptorSetLayout& lay) {
											 vkDestroyDescriptorSetLayout(dev, lay, nullptr);
										 });
		}
		...
		{
			std::vector<VkDescriptorSetLayout> descriptorLayouts = {dsLayout.get()};
			std::vector<VkPushConstantRange> pushConstantRanges = {
				{VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(ModelPushConstantBlock) + sizeof(ShadowPushConstantBlock)}};
			pipelineLayout = Managed(
				createPipelineLayout(context.get_device(), descriptorLayouts, pushConstantRanges),
				[dev = context.get_device()](VkPipelineLayout& lay) { vkDestroyPipelineLayout(dev, lay, nullptr); });
		}

		pipelineOmni =
			Managed(createGraphicsPipeline(context.get_device(), pipelineLayout.get(), renderPassOmni.get(),
										   {POINT_SHADOW_MAP_SIZE, POINT_SHADOW_MAP_SIZE}, "shaders/vShadow.vert.spv",
										   "shaders/fShadow.frag.spv", {VK_DYNAMIC_STATE_VIEWPORT},
										   VK_CULL_MODE_BACK_BIT, VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS),
					[dev = context.get_device()](VkPipeline& pipe) { vkDestroyPipeline(dev, pipe, nullptr); });

		pipelineDirectional =
			Managed(createGraphicsPipeline(context.get_device(), pipelineLayout.get(), renderPassDirectional.get(),
										   {DIR_SHADOW_MAP_SIZE, DIR_SHADOW_MAP_SIZE}, "shaders/vDirShadow.vert.spv",
										   "shaders/fDirShadow.frag.spv",
										   {VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_DEPTH_BIAS},
										   VK_CULL_MODE_BACK_BIT, VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS),
					[dev = context.get_device()](VkPipeline& pipe) { vkDestroyPipeline(dev, pipe, nullptr); });
...

		{
			...
			vkUpdateDescriptorSets(context.get_device(), static_cast<uint32_t>(writes.size()), writes.data(), 0,
								   nullptr);
		}
	}
...
LightSystem::LightSystem(LightSystem&& other) noexcept
	: renderPassOmni(std::move(other.renderPassOmni)), renderPassDirectional(std::move(other.renderPassDirectional)),
	  pipelineLayout(std::move(other.pipelineLayout)), pipelineOmni(std::move(other.pipelineOmni)),
	  pipelineDirectional(std::move(other.pipelineDirectional)), dsPool(std::move(other.dsPool)),
	  dsLayout(std::move(other.dsLayout)), uboDescriptorSet(std::move(other.uboDescriptorSet)),
	  shadowDescriptorSet(std::move(other.shadowDescriptorSet)), shadowLayout(std::move(other.shadowLayout)),
	  viewsUBO(std::move(other.viewsUBO)), pointShadows(std::move(other.pointShadows)),
	  pointShadowFreeStack(std::move(other.pointShadowFreeStack)),
	  pointShadowHandleValidity(std::move(other.pointShadowHandleValidity)), dirShadows(std::move(other.dirShadows)),
...
void LightSystem::cast(const Context& context, Camera* camera, VkCommandBuffer cmdBuffer,
