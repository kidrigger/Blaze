...
public:
	...
	PointShadow(const Context& context, VkRenderPass renderPass) noexcept
	{
		...
		{
			std::vector<VkImageView> attachments = {shadowMap.get_imageView(), depthMap.get_imageView()};
	...
	const TextureCube& get_shadowMap() const
	{
		return shadowMap;
	}
	TextureCube& get_shadowMap()
	{
		return shadowMap;
	}
...
public:
	...
	DirectionalShadow(const Context& context, VkRenderPass renderPass) noexcept
	{
		...
		framebuffers = util::ManagedVector(
			fbos, [dev = context.get_device()](VkFramebuffer& fbo) { vkDestroyFramebuffer(dev, fbo, nullptr); });
	}

	const VkFramebuffer& get_framebuffer(uint32_t idx) const
	{
	...
	const Texture2D& get_shadowMap() const
	{
		return shadowMap;
	}
	Texture2D& get_shadowMap()
	{
		return shadowMap;
	}
...
public:
	...
	{
		vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, set, 1, &shadowDescriptorSet.get(),
								0, nullptr);
	}
...
??? from here until ???END lines may have been inserted/deleted
	...
	*/
	...
	/**
	 * @fn cast(const Context& context, Camera* camera, VkCommandBuffer cmdBuffer, const std::vector<Drawable*>&
	 * drawables)
	 *
	 ...
	 */
	void cast(const Context& context, Camera* camera, VkCommandBuffer cmdBuffer,
	 ...
	 * @{
	 */
	const VkDescriptorSetLayout& get_shadowLayout() const
	{
		return shadowLayout.get();
	}
	/**
	 * @}
	 */
...
private:
	...
	CascadeBlock createCSMShadowPCB(const DirectionalShadow& shadow, Camera* camera) const
	{
		...
		CascadeBlock cb;
		cb.splits = splits;
		cb.splits[3] = static_cast<float>(shadow.numCascades);
		...
		for (float plane : cascadeSplits)
		{
			...
			auto lightViewMatrix = glm::lookAt(center + 2.0f * bz * r - bz * (shadow.nearPlane + shadow.farPlane),
	...
	ShadowPushConstantBlock createDirShadowPCB(const DirectionalShadow& shadow, Camera* camera) const
	{
		...
		glm::vec3 center =
			camera->get_direction() * centerDist(camera->get_nearPlane(), camera->get_farPlane(), cosine);

		float r = glm::distance(glm::vec3(frustumCorners[0]), center);

		auto& bz = shadow.direction;
		return {glm::ortho(-r, r, -r, r, shadow.nearPlane, shadow.farPlane) *
					glm::lookAt(center + 2.0f * bz * r - bz * (shadow.nearPlane + shadow.farPlane), center,
								glm::vec3(0, 1, 0)),
				shadow.direction};
	}
	...
	ShadowHandle createPointShadow(const glm::vec3& position = glm::vec3(0.0f), float nearPlane = 1.0f,
								   float farPlane = 20.0f)
	{
	...
	ShadowHandle createDirShadow(const glm::vec3& direction = glm::vec3(0, -1, 0), float width = 64.0f,
								 float height = 64.0f, float nearPlane = 1.0f, float farPlane = 512.0f)
	{
	...
	void cast(const Context& context, ShadowHandle handle, PointShadow& shadow, VkCommandBuffer cmdBuffer,
			  const std::vector<Drawable*>& drawables)
	{
		...
		vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout.get(), 0, 1,
								uboDescriptorSet.data(), 0, nullptr);
		vkCmdPushConstants(cmdBuffer, pipelineLayout.get(), VK_SHADER_STAGE_VERTEX_BIT, sizeof(ModelPushConstantBlock),
						   sizeof(ShadowPushConstantBlock), &shadowPCB);
	...
	void cast(const Context& context, ShadowHandle handle, DirectionalShadow& shadow, Camera* cam,
			  VkCommandBuffer cmdBuffer, const std::vector<Drawable*>& drawables)
	{
		shadow.get_shadowMap().implicitTransferLayout(VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
		...
		for (uint32_t i = 0; i < shadow.numCascades; i++)
		{
			...
			vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout.get(), 0, 1,
									uboDescriptorSet.data(), 0, nullptr);
			vkCmdPushConstants(cmdBuffer, pipelineLayout.get(), VK_SHADER_STAGE_VERTEX_BIT,
							   sizeof(ModelPushConstantBlock), sizeof(ShadowPushConstantBlock), &shadowPCB);
		...
		shadow.get_shadowMap().transferLayout(cmdBuffer, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL,
