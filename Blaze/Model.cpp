...
namespace blaze {
    ...
    Material::Material(MaterialPushConstantBlock pushBlock, Texture2D&& diff, Texture2D&& norm, Texture2D&& metal, Texture2D&& ao, Texture2D&& em)
    ...
    Node::Node() noexcept :
        ...
        primitive_range(0, 0), localTRS(1.0f) {}

    Node::Node(const glm::vec3& trans, const glm::quat& rot, const glm::vec3& sc, const std::vector<int>& children, const std::pair<int, int> primitive_range) noexcept :
        translation(trans), rotation(rot), scale(sc),
        children(children), primitive_range(primitive_range)
    {
    ...
    Model::Model(const Renderer& renderer, const std::vector<int>& top_level_nodes, std::vector<Node>& nodes, std::vector<Primitive>& prims, std::vector<Material>& mats, IndexedVertexBuffer<Vertex>&& ivb) noexcept
