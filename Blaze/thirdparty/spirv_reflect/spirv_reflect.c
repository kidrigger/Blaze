...
static SpvReflectResult IntersectSortedUint32(const uint32_t* p_arr0, size_t arr0_size, const uint32_t* p_arr1,
											  size_t arr1_size, uint32_t** pp_res, size_t* res_size)
{
	*res_size = 0;
...
static SpvReflectResult ReadStr(Parser* p_parser, uint32_t word_offset, uint32_t word_index, uint32_t word_count,
...
static SpvReflectResult ParseNodes(Parser* p_parser)
{
	...
	if (p_parser->access_chain_count > 0)
	{
		p_parser->access_chains =
			(AccessChain*)calloc(p_parser->access_chain_count, sizeof(*(p_parser->access_chains)));
		...
		case SpvOpAccessChain: {
			...
			if (p_access_chain->index_count > 0)
			{
				p_access_chain->indexes =
					(uint32_t*)calloc(p_access_chain->index_count, sizeof(*(p_access_chain->indexes)));
				...
				for (uint32_t index_index = 0; index_index < p_access_chain->index_count; ++index_index)
				{
					// Read index id
					uint32_t index_id = 0;
					CHECKED_READU32(p_parser, p_node->word_offset + SPIRV_ACCESS_CHAIN_INDEX_OFFSET + index_index,
									index_id);
...
static SpvReflectResult ParseType(Parser* p_parser, Node* p_node, Decorations* p_struct_member_decorations,
...
static SpvReflectResult ParseDescriptorBindings(Parser* p_parser, SpvReflectShaderModule* p_module)
{
	...
	p_module->descriptor_bindings = (SpvReflectDescriptorBinding*)calloc(p_module->descriptor_binding_count,
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(Parser* p_parser, SpvReflectShaderModule* p_module,
														  ...
														  SpvReflectBlockVariable* p_var)
{
	...
	for (uint32_t member_index = 0; member_index < p_var->member_count; ++member_index)
	{
		...
		case SpvOpTypeStruct: {
			SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser, p_module, false, is_parent_aos,
																		is_parent_rta, p_member_var);
			if (result != SPV_REFLECT_RESULT_SUCCESS)
			{
				return result;
			}
...
static SpvReflectResult ParseDescriptorBlockVariableUsage(Parser* p_parser, SpvReflectShaderModule* p_module,
														  AccessChain* p_access_chain, uint32_t index_index,
...
static SpvReflectResult ParseDescriptorBlocks(Parser* p_parser, SpvReflectShaderModule* p_module)
{
	...
	for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count; ++descriptor_index)
	{
		...
		result =
			ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false, is_parent_rta, &p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
...
static SpvReflectResult ParseInterfaceVariable(Parser* p_parser, const Decorations* p_type_node_decorations,
											   SpvReflectShaderModule* p_module, SpvReflectTypeDescription* p_type,
											   SpvReflectInterfaceVariable* p_var, bool* p_has_built_in)
{
	...
	if (p_type->member_count > 0)
	{
		...
		for (uint32_t member_index = 0; member_index < p_type_node->member_count; ++member_index)
		{
			...
			SpvReflectResult result = ParseInterfaceVariable(p_parser, p_member_decorations, p_module, p_member_type,
...
static SpvReflectResult ParseInterfaceVariables(Parser* p_parser, SpvReflectShaderModule* p_module,
												SpvReflectEntryPoint* p_entry, size_t io_var_count, uint32_t* io_vars)
{
	if (io_var_count == 0)
	{
		return SPV_REFLECT_RESULT_SUCCESS;
...
static SpvReflectResult ParseStaticallyUsedResources(Parser* p_parser, SpvReflectShaderModule* p_module,
													 SpvReflectEntryPoint* p_entry, size_t uniform_count,
													 uint32_t* uniforms, size_t push_constant_count,
...
static SpvReflectResult ParseEntryPoints(Parser* p_parser, SpvReflectShaderModule* p_module)
{
	...
	p_module->entry_points =
		(SpvReflectEntryPoint*)calloc(p_module->entry_point_count, sizeof(*(p_module->entry_points)));
	...
	for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i)
	{
		...
		result =
			ParseInterfaceVariables(p_parser, p_module, p_entry_point, interface_variable_count, interface_variables);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
		...
		result = ParseStaticallyUsedResources(p_parser, p_module, p_entry_point, uniform_count, uniforms,
...
const SpvReflectInterfaceVariable* spvReflectGetEntryPointInputVariableByLocation(
	const SpvReflectShaderModule* p_module, const char* entry_point, uint32_t location, SpvReflectResult* p_result)
{
	if (location == INVALID_VALUE)
	{
		if (IsNotNull(p_result))
...
const SpvReflectInterfaceVariable* spvReflectGetEntryPointInputVariableBySemantic(
	const SpvReflectShaderModule* p_module, const char* entry_point, const char* semantic, SpvReflectResult* p_result)
{
	if (IsNull(semantic))
	{
		if (IsNotNull(p_result))
...
SpvReflectResult spvReflectChangeDescriptorBindingNumbers(SpvReflectShaderModule* p_module,
														  ...
														  uint32_t new_binding_number, uint32_t new_set_binding)
{
	if (IsNull(p_module))
	{
		return SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
